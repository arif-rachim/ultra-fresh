...
import {blue, blueDarken, ButtonTheme, white} from "./Theme";
...
export function SignIn(props: RouteProps) {
    ...
    return <div style={{height: '100%', overflow: 'auto', display: 'flex', flexDirection: 'column'}}>
        <div style={{display:'flex',flexDirection:'row-reverse'}}>
            <motion.div style={{fontSize:36,margin:20,color:'rgba(0,0,0,0.6)'}} whileTap={{scale:0.95}} onTap={() => window.history.back()}>
                <IoClose />
            </motion.div>
        </div>
        <div style={{display: 'flex', justifyContent: 'center',marginTop:30}}>
            <motion.div style={{background: 'rgba(0,0,0,0.03)', margin: 10, padding: 10, borderRadius: 5}}
                        animate={{background:loginWithOtp?blue:white,
        ...
        <motion.div layout style={{display: 'flex', flexDirection: 'column',margin:30}}>
            ...
            {!loginWithOtp &&
                <motion.div layout>
                    <Input title={'Password'} placeholder={'Please enter password'} style={{containerStyle:{borderBottom:'none'}}}/>
                </motion.div>
            }
            <Button onTap={() => {
            }} title={loginWithOtp?'Send OTP':'Sign In'} icon={loginWithOtp?MdSms:IoLogInOutline} theme={ButtonTheme.promoted} style={{marginTop: 10}}/>
        </motion.div>
    </div>
}